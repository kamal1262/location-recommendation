...
--   WHERE 
--     (IF(filter_location1!="All", lower(filter_location1), lower(level_1)) = lower(level_1))
--     AND (IF(filter_location2!="All", lower(filter_location2), lower(level_2)) = lower(level_2))
--     AND (IF(filter_location3!="All", lower(filter_location3), lower(level_3)) = lower(level_3))
),
...
FILTERING_SEARCH AS (
--   SELECT DISTINCT A.*
  SELECT DISTINCT A.clientId, A.visitMYDateTime, A.visitId, A.visitNumber, A.hitNumber
  ...
  LEFT JOIN FLATTEN_LOC_DATA C ON (NULLIF(TRIM(A.level_3), "") IS NULL) AND (A.level_2 = LOWER(C.city_display_name)) AND (A.level_1 = LOWER(C.state_display_name))
  LEFT JOIN FLATTEN_LOC_DATA D ON (NULLIF(TRIM(A.level_3), "") IS NULL) AND (NULLIF(TRIM(A.level_2), "") IS NULL) AND (A.level_1 = LOWER(D.state_display_name))
)

SELECT * FROM FILTERING_SEARCH
WHERE (COALESCE(
